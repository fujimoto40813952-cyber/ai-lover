...
export default function TermsPage() {
  return (
    <LegalShell title="利用規約" updatedAt="2026年6月17日">
      <p>
        本利用規約（以下「本規約」）は、【要記入：運営事業者名】（以下「当社」）が提供するAIコンパニオンサービス「AI Lover」（以下「本サービス」）の利用条件を定めるものです。利用者（以下「ユーザー」）は、本サービスを利用することで本規約に同意したものとみなされます。
      </p>
      ...
      <p>本規約は日本法に準拠します。本サービスに関して紛争が生じた場合、【要記入：管轄裁判所（例：東京地方裁判所）】を第一審の専属的合意管轄裁判所とします。</p>

      <h2 className="text-xl font-semibold text-white">第12条（お問い合わせ）</h2>
      <p>本規約に関するお問い合わせは【要記入：連絡先メールアドレス】までご連絡ください。</p>
    </LegalShell>
  )
}
